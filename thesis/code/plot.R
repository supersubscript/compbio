...
###############################################################################
plot.lineage = function(lineage.sublines.data, var="Mean.cell.intensity", ylim=c(0,160), batch = 1) {
  # plot(NA, xlim=c(0,84), ylim=ylim)
  plot(NA, xlim = c(0, 84), ylim = ylim)
  for (ii in 1:length(lineage.sublines.data)) 
    lines(lineage.sublines.data[[ii]][, "Time"], 
          lineage.sublines.data[[ii]][, var], type = "b", lwd = lw.s, col = ii)
  plot(NA, xlim = c(0, 84), ylim = c(0, 7))
  for (ii in 1:length(lineage.sublines.data)) 
    lines(lineage.sublines.data[[ii]][, "Time"], 
          lineage.sublines.data[[ii]][, "dist2top"], type = "b", lwd = lw.s, col = ii)
}
