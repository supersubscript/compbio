...
pathToFiles_segmented='../../data/clv3_complete/PNAS/plant2/segmentation_tiffs/'; %insert full path here
...
%%
% Get region properties for nuclear/cellular segmentation
...
%%
% Get all centroids for cells segmented using MARS

centroids_mars_cells={};
...
for k=1:size(filelist_segmented,1)
    ...
    for m=1%:size(allFrames{1,i},1) 
        ...
        for j=2:size(nr,2)
            ...
            centroids_mars_cells{k}(j,1)=nanmean(allPixels(:,1));
            centroids_mars_cells{k}(j,2)=nanmean(allPixels(:,2));
            centroids_mars_cells{k}(j,3)=nanmean(allPixels(:,3));
...
centroids_mars_cells_corrected={};
centroids_costanza_corrected={};

for i=1:size(centroids_mars_cells,2)
    
    for j=1:size(centroids_mars_cells{i},1)
        
        centroids_mars_cells_corrected{i}(:,1)=centroids_mars_cells{i}(:,1).*pixelsizes(1,i);
        centroids_mars_cells_corrected{i}(:,2)=centroids_mars_cells{i}(:,2).*pixelsizes(2,i);
        centroids_mars_cells_corrected{i}(:,3)=centroids_mars_cells{i}(:,3).*pixelsizes(3,i);
...
for i=1:size(centroids_costanza_corrected,2)
    ...
    for j=1:size(centroids_costanza_corrected{i},1)
        ...
        for k=1:size(centroids_mars_cells_corrected{i},1)
            
            xv=centroids_mars_cells_corrected{i}(k,1);
            yv=centroids_mars_cells_corrected{i}(k,2);
            zv=centroids_mars_cells_corrected{i}(k,3);
...
%%
% Write correspondence tables and centroid lists
time=[0:4:84];
for i=1:size(correspondences,2)
    
    fname = sprintf('ID_correspondence_plant#13_%d.txt',time(i));
...
for i=1:size(centroids_mars_corrected,2)
    ...
    fname = sprintf('centroids_mars_cells_plant#4_%d.txt',time(i));
...
for i=1:size(centroids_costanza_corrected,2)
    ...
    fname = sprintf('centroids_costanza_plant#4_%d.txt',time(i));
