...
library(knitr)
opts_chunk$set(fig.path='figure/twocolumn-', fig.align='center', fig.show='hold',engine='python') 
render_listings()
setwd("~/compbio/src/assignments/gsa1/report/")
@
...
output = read_and_output(mat_file, idist_file, 115, True, False)
import textwrap
print '\n'.join(textwrap.wrap(output, 80, break_long_words=True))
@
...
random.seed(2)
# print "emit:"
# print '\n'.join(textwrap.wrap(emit, 80, break_long_words=True))
# print "\nhidden:"
# print '\n'.join(textwrap.wrap(hidden, 80, break_long_words=True))
plt.yticks([])
pl_emit, = plt.step(xrange(len(emit)), list(emit), label = "Emit")
pl_hidd, = plt.step(xrange(len(hidden)), list(int(s) +4.3 for s in hidden), label = "Hidden")
plt.legend(handles=[pl_emit, pl_hidd])
fig = plt.gcf()
fig.set_size_inches(6, 4)
fig.savefig("/home/henrik/compbio/src/assignments/gsa1/report/hidden_emit.png")
@
...
pl_emit,__,__ = plt.hist(list(int(s) for s in model3_seq), bins=5, alpha=0.5, label='Emitted sequence')
pl_hidd,__,__ = plt.hist(list(int(s) for s in relab), bins = 5, alpha=0.5, label='Genome classification')
plt.legend()
plt.xlabel("Category")
...
fig = plt.gcf()
fig.set_size_inches(6, 4)
fig.savefig("/home/henrik/compbio/src/assignments/gsa1/report/gc_emit.png")
@
...
\subsection*{6}
<<engine="python", echo = 230:289, eval=FALSE>>=
#!/usr/bin/env python
import numpy as np
import random
import math
...
fig = plt.gcf()
fig.set_size_inches(6, 4)
fig.savefig("/home/henrik/compbio/src/assignments/gsa1/report/viterbi.png")
@
\begin{figure}[H]
  \centering
