...
par(mfrow = c(3, 3))
no.breaks = 10
for(ii in 6:15) if(!all(is.na(all.data[,ii]))) hist(all.data[, ii], breaks = no.breaks, main = colnames(all.data)[ii], freq = F)

##################
### Plot all z-values
##################
par(mfrow = c(1, 5))
for(plant in plants[-which(plants == 1)]) {
  plot(NA, xlim = c(0, 84), ylim = c(0, 10))
  for (ii in 1:length(zs[, as.character(plant)][[1]])) {
    points(rep((ii - 1) * 4, length(zs[, as.character(plant)][[1]][[ii]])), zs[, as.character(plant)][[1]][[ii]])
  }
}
