...
% last param is theta
tt = (0:0.1:1);
for t_ind = 1:length(tt)
    for pc_ind = 1:length(tt)
        theta = tt(t_ind);
        pc_weight = tt(pc_ind);
        covariance_analysis2(horzcat(name, '.fas'), protein, horzcat(name, '_MI_DIs_', protein, '_theta_', num2str(theta), '_pc_weight_', num2str(pc_weight), '.txt' ), pc_weight,'N','Y', theta, pc_weight, protein, name);
