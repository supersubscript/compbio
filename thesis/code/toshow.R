...
results   = parLapply(cl, plants, function(x) get.plant.data(x, only.l1 = TRUE))
...
plant = 4; cell.lines.data = lapply(results[[plant]]$lineage.sublines.data, collapse.lineage) 
...
lineage.sublines.data = lineage.sublines.data[sapply(lineage.sublines.data, function(x) sum(nna(unlist(x)))) > 6*8]
plot.12(unlist(lineage.sublines.data, recursive = F), batch = 1)
par(mfcol = c(2, 3)); for(ii in 1:3) plot.lineage(lineage.sublines.data[[ii]], ylim = c(0, 160))
