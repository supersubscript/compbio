...
qs1 = replicate(no.replicates, run.func(.05))
qs2 = replicate(no.replicates, run.func(.1))
qs3 = replicate(no.replicates, run.func(.2))
...
out = apply(qs2[, -1], 2, function(x)
  lines(x, col = "firebrick"))
out = apply(qs3[, -1], 2, function(x)
  lines(x, col = "forestgreen"))
legend("topright", c("0.05", "0.1", "0.2"), col = c("orange", "firebrick", "forestgreen"), lty=c(1,1,1), cex = 1.2, lwd = 3)
