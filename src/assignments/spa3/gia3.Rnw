...
\section*{Results}
<<echo = FALSE, cache = TRUE, message = FALSE, warning = FALSE, results = 'hide'>>=
setwd("~/gia3")
data = ReadAffy(celfile.path = paste0(getwd(), "/../GSE21822")) # Read in data
pData(phenoData(data)) = cbind(pData(phenoData(data)),group = c("XX","XX","XX","X[Y-]sry","X[Y-]sry","X[Y-]sry","XXsry","XXsry","XXsry","X[Y-]", "X[Y-]","X[Y-]"))
...
<<echo = FALSE, cache = TRUE, warning=FALSE, message = FALSE, include = FALSE>>=
# Female comparison
...
results.xyxxsry = getResults("XXsry","X[Y-]")

plotPairwiseDiffExp = function(x)
{
  plot(x@means[,1], x@means[,2], cex = .4, col = 'lightgray', xlab = colnames(x@means)[1], ylab = colnames(x@means)[2], pch = 15)
  lines(0:40, 0:40 + .2, lw = 2, lt = 2, col = 'black')
  lines(0:40, 0:40 - .2, lw = 2, lt = 2, col = 'black')
  points(means(x)[which(rownames(means(x)) %in% scs.genes), ], col = 'firebrick',pch = 16,cex = .7)
  points(means(x)[which(rownames(means(x)) %in% dimorphic.regulated.by.cc), ], col = 'skyblue',pch = 17,cex = .8)
  #points(means(x)[which(rownames(means(x)) %in% intersect(scs.genes, dimorphic.regulated.by.cc)), ], col = 'skyblue', pch = 16,cex = .8)
}
@
...
To determine whether sex-chromosome sensitivity (SCS) affects autosomal gene expression over the whole genome, we produce the comparison seene in \cref{fig:venns}A and B where differences between the genotypes of the males (A) and females (B) are shown. Using our definition of significant expression values, we are able to round up \Sexpr{length(scs.genes)} SCS genes, as opposed to Wijchers et al.\ who find 369 differentially expressed genes under the same conditions. Nevertheless, we are able to produce the same trends in the expression, namely that our SCS genes are biased towards an \XX configuration at its core in males, and the contrary in females. Similarly, we establish a set of sex-sensitive and dimorphic genes (SSD) comparing \XX and \XYSRY individuals, and are able to find a selection of \Sexpr{length(dimorphic.regulated.by.cc)} (175) such genes. We also identify \Sexpr{length(dimorphic.in.either.sex)} (401) diti in either sex. A list of the genes contained in each group can be provided upon request.
...
<<venns, echo=FALSE, fig.height = 2., fig.width=3.7, fig.env='figure', fig.cap="Venn representations of genes differentially expressed in various groups of interest, and their corresponding overlaps.", cache = TRUE, fig.pos='H'>>=
library(VennDiagram)
venn.male.female = venn.diagram(list(Male = rownames(means(significant.male)), Female=rownames(means(significant.female))), fill = 2:3, alpha = .3, filename = NULL, cat.dist = c(.06, .06), cat.pos = c(7, 4), width = 800, height = 1000)
venn.scs.dimorph = venn.diagram(list(SCS = scs.genes, Dimorphic = rownames(means(significant.dimorphic))), fill = 4:5, alpha = .3, filename = NULL, cat.dist = c(.05, .04), cat.pos = c(1, 2), width = 800, height = 1000)
grid.arrange(gTree(children = venn.male.female), gTree(children = venn.scs.dimorph), ncol = 2)
grid.text("A", .05, .9, gp=gpar(fontsize = 17))
grid.text("B", .55, .9, gp=gpar(fontsize = 17))
@ 
...
<<snp, echo=FALSE, fig.height = 2.2, fig.env='figure', fig.cap="Known SNPs related to disease or changes in phenotype. Axis shows chromosome coordinates.", cache = TRUE, fig.pos="H",fig.align='center'>>=
par(mar=c(3,3,1,1))
...
rect(2787150, .85, 2787480, 1.15, lwd = 3)
points(data[, "Chr"],
       rep(1, nrow(data)),
       type = "p",
...
text(start   + 75, 1.2, "SRY", cex = 1.5)
text(2787150 + 100, 1.3, "HMG-box", cex = 1.5)
@
...
text(2.5,12, "E", cex = 2)
plotPairwiseDiffExp(results.xyxxsry)
text(2.5,12, "F", cex = 2)
@
