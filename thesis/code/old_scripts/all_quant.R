...
################################
par(mfrow = c(3,2))
for(ii in 1:nrow(no.clv3.nuclei)) {
  plot(timepoints[ii,], no.clv3.nuclei[ii,], type = "b", main = "no.clv3.nuclei", xlim=c(0,84))
  for(jj in darktimes) abline(v=jj, untf=FALSE, cex = .1, lty = 1)
}
