          ...
          for (rep in 1:technical.replicates) {
           ...
           for (ii in 1:no.iterations) {
            ...
            if (any(apply(patterns, 2, function(x)
              output.pattern(output, x))) & output == patterns[,this.pattern]) {
...
trimmed = lapply(data, "[")
ll = vector("list", length=length(weights.removed))
for(ii in 1:length(trimmed)){
  ll[[ii %% length(ll) + 1]] = append(ll[[ii %% length(ll) + 1]], unlist(trimmed[[ii]]))
}
temp = ll[[1]]
ll = ll[-1]
ll = append(ll, c())
ll[[length(weights.removed)]] =  temp
mat.ll = do.call(rbind, ll)

par(mfrow=c(1,1))
avg = apply(mat.ll,2, mean)
serr = apply(mat.ll,2, function(x) sd(x) / sqrt(length(x)))
plot(avg, pch = 19, xaxt = "n", bty ="n", yaxt = "n", ylab = "", xlab = "")
lines(avg)
axis(1, cex = 2, labels = weights.removed, at=1:length(avg))
axis(2, cex = 2)
mtext("Fraction weights removed", side=1, las = 1, line = 2.5, cex = 1.5)
...
# legend("topleft", c("Hebb", "Storkey"), pch = c(19, 19), col = c("red", "blue"), inset = .03, cex = 1.5, box.lwd = 0,box.col = "white",bg = "white")

#save(data, file = "hebb.RData")
# 
# load("hebb.RData")
# par(mfrow=c(2,1), oma = c(6,5,4,2) + 0.0,
#     mai = c(.0,.6,.0,.5))
...
# plot(no.patterns.stored, avg, pch = 19, xlab = "",ylab = "", bty = "l", xaxt = "n", yaxt="n", ylim=c(2,13), xlim = c(2,25), col = "red")
# axis(side = 2, cex = 1.5, labels = 4:13, at=4:13)
# arrows(no.patterns.stored, avg-serr, no.patterns.stored, avg+serr, length=0.05, angle=90, code=3, cex = 2, col = "red")
# legend("topleft", c("Hebb", "Storkey"), pch = c(19, 19), col = c("red", "blue"), inset = .03, cex = 1.5, box.lwd = 0,box.col = "white",bg = "white")

# load("storkey.RData")
# avg = apply(sapply(data, function(x) unlist(x)), 1, mean)
# serr = apply(sapply(data, function(x) unlist(x)), 1, function(x) sd(x, na.rm = TRUE) / sqrt(length(x)))
# plot(no.patterns.stored, avg, pch = 19, xlab = "", ylab = "", bty = "l", xaxt = "n", yaxt="n", ylim=c(2,25), xlim = c(2,25), col = "blue")
# axis(side = 1, cex = 1.5) 
# axis(side = 2, cex = 1.5)
# mtext("Patterns retrieved", side=2, las = 3, line = 2.5, cex = 1.5, adj = -1.5)
# mtext("Patterns stored", side=1, las = 1, line = 2.5, cex = 1.5, adj = .5)
# arrows(no.patterns.stored, avg-serr, no.patterns.stored, avg+serr, length=0.05, angle=90, code=3, cex=2, col = "blue")
