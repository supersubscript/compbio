...
get.plant.data = function(plant.no){
  ...
  for (time in 2:length(mapping.files)) {
    ...
    for (ii in 1:length(mapping.data)) {
      ...
      if (!added) {
        new.cell.line = list(c(sapply(1:(time - 1), function(x) list(NA)), list(mapping.data[[ii]][1])))
  ...
  cell.lines = cell.lines[order(-apply(cell.lines, 1, function(x) length(unlist(x)[!is.na(unlist(x))]))), ]
      ...
      for (jj in 1:no.track.files) {
        ...
        if (length(cells.in.lineage) > 0){
          for (kk in 1:length(cells.in.lineage))
            lineage.data = rbind(lineage.data, c(Time = time.interval.ends[jj + 1], as.vector(quant.data[[jj]][cells.in.lineage[kk], ])))
        } else
  ...
  mothers = lapply(1:length(all.mapping.data), function(x)
            lapply(all.mapping.data[[x]], function(y) {
              ...
              # 
              if (length(cell.id) == 1)
                return(matrix(c(t = time.interval.ends[x + 1], mother.id, cell.id), ncol = 3, nrow = 1))
              
              mums = rep(mother.id, length(cell.id))
              ts = rep(time.interval.ends[x], length(cell.id))
              return(cbind(ts, mums, cell.id))
            }))
  mothers = do.call(rbind, unlist(mothers, recursive = F))
  first.gen = cbind(rep(time.interval.ends[1], length(all.mapping.data[[1]])),
  ...
  colnames(output.data) = c("cell.id", "lineage.id", "mother.id", "daughter1.id", 
                            ...
                            "dist2top", "daughter1.expr", "daughter2.expr")
  for(time in 1:length(mapping.files)){
    ...
    topcell.coords = c(unname(unlist(top.coords[order(-top.coords$z, top.coords$y ** 2 + top.coords$x ** 2), ][1, ])))
    ...
    for(ii in 1:length(division.data)){
      ...
      if(plant.no != 1){
        is.cell = all.quant.data$Cell.id[, time] %in% division.data[[ii]][1]
        is.daughter1 = all.quant.data$Cell.id[, time] %in% division.data[[ii]][2]
        is.daughter2 = all.quant.data$Cell.id[, time] %in% division.data[[ii]][3]
        
        # Does this cell exist in the quantified data?
        if (any(is.cell)) {
          xyzvid = c(
            all.quant.data$x[which(is.cell), time],
            all.quant.data$y[which(is.cell), time],
            all.quant.data$z[which(is.cell), time],
            all.quant.data$Boa.volume[which(is.cell), time],
            all.quant.data$Mean.cell.intensity[which(is.cell), time],
            sqrt((all.quant.data$x[which(is.cell), time] - topcell.coords[1]) ** 2 +
                 (all.quant.data$y[which(is.cell), time] - topcell.coords[2]) ** 2 +
                 (all.quant.data$z[which(is.cell), time] - topcell.coords[3]) ** 2)
            )
        }
        ...
        if ((plant.no == 4 && time > 6) || (plant.no == 15 && time > 3)) {
          daughter1.expr = ifelse(sum(is.daughter1) != 0, all.quant.data$Mean.cell.intensity[which(is.daughter1), time], NA)
          daughter2.expr = ifelse(sum(is.daughter2) != 0, all.quant.data$Mean.cell.intensity[which(is.daughter2), time], NA)
        } else {
          daughter1.expr = ifelse(sum(is.daughter1) != 0, all.quant.data$Mean.cell.intensity[which(is.daughter1), time + 1], NA)
          daughter2.expr = ifelse(sum(is.daughter2) != 0, all.quant.data$Mean.cell.intensity[which(is.daughter2), time + 1], NA)
        }
      }
      ...
      mother.id = unname(mothers[mothers[, "cell.id"] == this.cell.id & mothers[, "t"] == time.interval.ends[time], "mother.id"]) # Which is the mother id for this cell?
      ...
      for (kk in (time+1):2) {
        ...
        reference.id = mothers[mothers[, "cell.id"] == last.mother &
                               mothers[, "t"] == time.interval.ends[kk - 1], "mother.id"]
        is.not.na = reference.id[!is.na(reference.id)]
        if (length(is.not.na) == 0) 
          break
        is.present.previous = mothers[, "t"] == time.interval.ends[kk]
        ...
        no.offspring = length(mothers[is.ancestor & is.present.previous, "cell.id"])
        if (no.offspring > 1)
          break
        age = age + time.interval.ends[kk] - time.interval.ends[kk - 1]
      }
#       
# a =   (list(this.cell.id,
#         lineage.id,
...
#         division.data[[ii]][3],
#         age,
#         time.interval.ends[time],
#         xyzvid,
#         daughter1.expr,
#         daughter2.expr))
# a = which(sapply(unlist(a), function(x) length(x) == 0))
      # stopifnot(length(warnings()) == 0)
# print(length(unlist(a)))
# Sys.sleep(0.05)
# if(ncol(output.data) != 15)
# print(ncol(output.data))
