...
function get_constraints_PC(path, type,SCORING_METHOD_FOR_RANKING_CONSTRAINTS, theta, pc_weight, protein)
...
if type == 'P'
	DIScoreFilename = horzcat('../figures/', path,  '_theta_', theta, '_pc_weight_', pc_weight, '_L1_PSEUDO_DIScores.csv');
end
     
Predicted_Constraints_name = horzcat('../figures/', path,  '_theta_', theta, '_pc_weight_', pc_weight, '_Predicted_Constraints');
...
%if (strcmp(BATCH_MODE,'yes') ~= 1)
	...
	eval(['print -dpdf -f'  ' ' Predicted_Constraints_name '.pdf']);
%end
end
