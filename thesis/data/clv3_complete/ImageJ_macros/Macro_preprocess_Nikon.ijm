...
channel_set=3; 		// This is to select the set of channels to process. Note different microscopes will have different channels. This can be improved it the future, and to be read from the nd files.
colum_stages_row=8; // CHANGE IF NEEDED Row number in the nd file where the number of positions is found (not sure why is 10 and not 9?)

// Macro input strings

path_in=getDirectory("Choose Destination Directory ");
...
datafoldername="2015-04-29_Jose_cyano";
...
if (channel_set == 0)
    ...
    channels=newArray("w1Brightfield","w2TRITC");
...
//run("Fire");
if (slices > 1){
	saveAs("Tiff", fullpath_out+relevantstring+"SUMPROJECT.tif");}
else 
{saveAs("Tiff", fullpath_out+relevantstring+"IMAGE.tif");}
