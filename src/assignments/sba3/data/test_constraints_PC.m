...
if (strcmp(BATCH_MODE,'yes') ~= 1)
    ...
    eval(['print -dpdf -f'  ' ' FPoutputfile '.pdf']);
...
if (strcmp(BATCH_MODE,'yes') ~= 1)
    ...
    csvwrite(horzcat('../figures/', path, '_theta_', theta, '_pc_weight_', pc_weight, '_crystal_contacts.csv'), [plotRows,plotCols]);
%     fa = figure('visible','off');
    ...
    csvwrite(horzcat('../figures/', path, '_theta_', theta, '_pc_weight_', pc_weight, '_prediction_contacts.csv'), [plotRows,plotCols]);
...
%     fb = figure('visible','off');
    ...
    eval(['print -dpdf -f'  ' ' horzcat(Cmapoutputfile,'_',num2str(CONTACT_MAP_EIC_COUNT)) '.pdf']);
    eval(['print -dpdf -f' ' ' EICoutputfile '.pdf']);
%     saveas(fa,horzcat(Cmapoutputfile,'_',num2str(CONTACT_MAP_EIC_COUNT)),'pdf')
%     saveas(fb,FPoutputfile,'pdf')
