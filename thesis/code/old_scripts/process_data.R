...
get.division.events = function(mapping.data, quant.data, sublines, plant.no){
  ...
  for(time in 1:length(mapping.data)){
    ...
    for(ii in 1:length(division.data)){
      ...
      if (plant.no != 1) {
        this.cell.index = match(division.data[[ii]][1], quant.data[[time]][, "Cell.id"])
        daughter1.index = match(division.data[[ii]][2], quant.data[[time + 1]][, "Cell.id"])
        daughter2.index = match(division.data[[ii]][3], quant.data[[time + 1]][, "Cell.id"])
        
        # Does this cell exist in the quantified data?
        xyzvid         = c(unlist(quant.data[[time]][this.cell.index, -1]))
        if(is.null(xyzvid))
          xyzvid = rep(NA, 6)
        daughter1.expr = quant.data[[time + 1]][daughter1.index, "Mean.cell.intensity"]
        daughter1.vol  = quant.data[[time + 1]][daughter1.index, "Boa.volume"]
        daughter2.expr = quant.data[[time + 1]][daughter2.index, "Mean.cell.intensity"]
        daughter2.vol  = quant.data[[time + 1]][daughter2.index, "Boa.volume"]
...
get.sublines = function(mapping.data, timepoints) {
  ...
  lines = do.call(rbind, unlist(sapply(mapping.data[[1]], function(mapping)
    lapply(mapping[-1], function(child) c(mapping[1], child))), recursive = F)) # From old code
  lines = cbind(lines, matrix(NA, ncol = length(timepoints) - 2, nrow = nrow(lines)))
  
  ### Add the rest
  for (time in 2:(length(mapping.data))) {
    ...
    lines = rbind(lines, new.lines)
  }

  colnames(lines) = timepoints

  lines = lines[do.call(order, as.data.frame(lines)), ]
  first.times = apply(lines, 1, function(x) min(which(nna(x))))
  first.vals  = unname(sapply(1:nrow(lines), function(x) lines[x, first.times[x]]))
  
  prev.first = -1; prev.time = -1; counter1 = 0; counter2 = 1
  rownames(lines) = rep(NA, NROW(lines))
  
  ### Set rownames according to lineage
  for (line in 1:nrow(lines)) {
    this.time = first.times[line]
    this.val  = first.vals[line]

    if (this.val == prev.first && this.time == prev.time) {
      counter2 = counter2 + 1
      rownames(lines)[line] = paste0(counter1, ".", counter2)
    } else {
      counter1 = counter1 + 1
      counter2 = 1
      rownames(lines)[line] = paste0(counter1, ".", counter2)
...
get.lineage.sublines.data = function(lineage, quant.data, timepoints) {
  ...
  for (subline in 1:length(members)) {
    ...
    for (time in 1:length(quant.data)) {
      # TODO: Fix this for plant 18
      q.time = ifelse(plant.no == 18  && timepoints[time] >= 40, time + 1, time)
      
      # If the member exists in the quantified data, add it
      qd.match = match(lineage[subline, time], quant.data[[q.time]][,"Cell.id"])
      if (nna(qd.match)) {
        sublines.data[[subline]][counter, ] =
          c(timepoints[time], unlist(quant.data[[q.time]][qd.match, ]))
        counter = counter + 1
      }
    }
...
###############################################################################
###############################################################################
get.lineages = function(sublines, sep = "\\."){
