...
labs(x = "Time [hours]", y = "Number of CLV3 nuclei", size= 14)
...
data %>% ungroup() %>%
  ...
  geom_point(aes(colour = factor(plant)), size = 3, alpha = .8, na.rm = TRUE) + theme_bw()  +
...
p1 = qd %>% filter(plant != 18) %>%
  ...
  scale_alpha_continuous(name = d2ap, range=c(0.05,.3))
...
p2 = qd %>% filter(plant %in% p, layer == "L1") %>%
  ...
  scale_alpha_continuous(name = "", guide = FALSE, range=c(0.05,0.3)) +
  scale_color_discrete(name = "Plant", guide = FALSE) + 
  theme(legend.position = "bottom") 
  
p3 = qd %>% 
  filter(!is.na(layer), plant %in% p, layer %in% c("L1", "L2")) %>%
  ggplot(aes(x = d2t, y = expr, colour = factor(plant), alpha = t)) +
  geom_point(na.rm = TRUE) +
  facet_grid(plant~ layer) +
  labs(x = expression(paste("Distance to apex [", mu, "m]")), y = "CLV3 expression") + 
  scale_y_continuous(position = "left") +
  scale_alpha_continuous(name = "Time [hours]",  range=c(0.05,0.3)) +
scale_color_discrete(name = "Plant", guide = FALSE) + 
  theme(legend.position = "bottom",
...
ggplot(f.data, aes(
  ...
  scale_fill_manual(
    name = "",
    breaks = c(3.5, 4.5),
    values = c(col1, col1, col1, col2, col1, col1, col1, col1, col1, col1, col1)
  ) +
...
data %>%
  ggplot(aes(x = d2t, y = expr, group = plant, colour = factor(plant), alpha = t)) +
  geom_point(na.rm = TRUE) +
  facet_wrap(~ plant) +
  theme_bw() +
  labs(x = expression(paste("Distance to apex [", mu, "m]")), y = "CLV3 expression") +
  scale_color_discrete(name= "Plant", guide = FALSE) + 
  scale_alpha_continuous(name = "Time [hours]", range = c(0.1, .7))
...
p2 = ggplot(cld %>% filter(lineage %in% select.lines), 
  ...
  labs(x = "Time [hours]", y = d2ap) + theme_bw() +
scale_color_discrete(guide=FALSE)
