...
addReaction(clv3)     = hill(act.ind = list(model.obj@species$pWUS@index, model.obj@species$pWUS@index + 2L), rep.ind = list(), vars = c(Cv, Ck, Cn, Ck2, Cn2))
addReaction(clv3)     = degradationOne(rate = Cd)
addSpecies(model.obj) = clv3

a                  = init.species("a")
addReaction(a) = creationOne(model.obj@species$aA@index, rate=ap)
addReaction(a) = degradationOne(rate=ad)
addReaction(a)     = diffusionSimple(rate = aD)
addReaction(a) = degradationTwo(index = model.obj@species$Sink@index, rate = aD)
addSpecies(model.obj) = a
...
init[, "a"] = c(1, 1, 0, 0, rep(0, no.cells - 4)) * 10
